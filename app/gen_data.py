...
def gen_data(collection):
    ...
    def generate_sample_data(num_samples):
        ...
        for _ in range(num_samples):
            ...
            class_attendance = random.choices(["Never", "Rarely", "Sometimes", "Often"], weights=[0.2, 0.3, 0.3, 0.2])[0]  
            attendance_mapping = {"Never": 4, "Rarely": 3, "Sometimes": 2,"Often": 1}
            num_class_attendance = attendance_mapping[class_attendance] / 4 * 5

            academic_impact = max(1, min(5, round(np.random.normal(3, 1) + noise(1))))  

            academic_pressure = max(1, min(5, round(academic_impact + noise(1.5))))  
            ...
            group_guilt = random.choices([1, 2, 3, 4, 5], weights=[0.3, 0.1, 0.1, 0.2, 0.3])[0]  
            
            attendance_impact = random.choices(["Positive", "Negative"], weights=[0.7, 0.3])[0]  
            additional_comments = ""
